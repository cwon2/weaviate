...
function build_binary() {
  ...
  GOOS=linux GOARCH=$arch go build -o $BUILD_ARTIFACTS_DIR/$arch/weaviate -ldflags "-w -extldflags \"-static\" -X github.com/cwon2/weaviate/usecases/config.GitHash='${GIT_HASH}'"
  step_complete
